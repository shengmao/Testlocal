...
#import "TEPatientMasterViewController.h"

@interface TEPatientMasterViewController ()
@property NSMutableArray *patientListArray;

@end

@implementation TEPatientMasterViewController
...
- (void)viewDidLoad
{
    ...
    NSArray *sectionOneArray = [NSArray arrayWithObjects:@"Bernd", @"Gustav", nil];
    NSDictionary *sectionOneDict = [NSDictionary dictionaryWithObject:sectionOneArray forKey:@"Table"];
    
    NSArray *sectionTwoArray = [NSArray arrayWithObjects:@"Heike", @"Manuel", nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell...
    if (cell == nil) {
        cell = [[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:@"PatientCell"];
    }
    ...
    patientnameTextfield.text = cellValue; //[array objectAtIndex:indexPath.row];
    UILabel *patientsurnameTextfield = (UILabel *) [cell viewWithTag:101];
    patientsurnameTextfield.text = @"Berthold";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //NSString *object = [self.patientListArray objectAtIndex:indexPath.row];
